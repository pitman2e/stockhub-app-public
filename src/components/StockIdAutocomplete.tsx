...
export default function StockIdAutocomplete({
  SetStateAction: StockDispatcher,
  onChange,
  portfolioId,
  stockId,
  isOpenPosOnly = false,
  ...
  return (
    <Autocomplete
      ...
      sx={{ minWidth: 300 }}
      defaultValue={
        stockId ? ({ stockId: stockId, stockName: "" } as IStock) : undefined
      }
      open={open}
      onOpen={() => {
        setOpen(true);
      ...
      getOptionLabel={(option) =>
        option.stockId + (option.stockName ? " - " : "") + option.stockName
      }
      options={data ?? []}
      loading={isLoading}
