...
describe("EditFormStock", () => {
  ...
  it("prevents submission and shows Yup validation errors on empty required fields (Stock Post)", async () => {
    ...
    expect(await screen.findByText(/^stockId .* required/i)).toBeInTheDocument();
    expect(await screen.findByText(/^stockName .* required/i)).toBeInTheDocument();
    expect(await screen.findByText(/^currency .* required/i)).toBeInTheDocument();
  });
