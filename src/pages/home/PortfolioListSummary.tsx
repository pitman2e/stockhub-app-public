...
export default function PortfolioListSummary() {
  const lsIsSumDetailExpanded =
    localStorage.getItem("homeIsSumDetailExpanded") === "Y";
  ...
  const lsDefaultCurrency =
    localStorage.getItem("homeDefaultCurrency") === null
      ? ""
      : localStorage.getItem("homeDefaultCurrency");
  const [defaultCurrency, setHomeDefaultCurrency] = useState(lsDefaultCurrency);

  const handleCurrencyChange: ToggleButtonGroupProps["onChange"] = (
    _event,
    newValue,
  ) => {
    const cur = newValue || "";
    ...
    <>
      ...
      <Paper>
        ...
        >
          ...
          <Grid size={{ xs: 3, sm: 6 }} sx={{ textAlign: "right" }}></Grid>

          <Grid
            container
            size={{ xs: 3, sm: 2 }}
            sx={{ textAlign: "right", flexDirection: "column" }}
          >
