...
function GridDetailItem({
  ...
  disablePosSign,
  currency,
}: IGridDetailItemProps) {
  const sxColor = (theme: Theme) =>
    !netAmtColor || !amount
      ? null
      : amount < 0
        ? theme.deltaColor.down
        : theme.deltaColor.up;

  const percentagePart = amountPercentage
    ? ` (${utils.getSignedDecimal(amountPercentage, 2, disablePosSign)}%)`
    : "";
