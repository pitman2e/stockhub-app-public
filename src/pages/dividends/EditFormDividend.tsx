...
import { IRealisedScripPutDto, IRealisedScripPutDtoSchema } from "../../types/api";
...
export default function EditFormDividend({
  ...
  const {
    ...
    resolver: yupResolver(IRealisedScripPutDtoSchema) as Resolver<IRealisedScripPutDto>,
  });
  const dispatch = useDispatch();
  const queryClient = useQueryClient();
  const saveMutation = useMutation({
    mutationFn: async (formData: IRealisedScripPutDto) => ({
      response: await ApiRequestAdapter.mutationOptions(repoStockRealisedScrip.Put()).requestFn(formData),
      invalidateQueryKey: ApiRequestAdapter.mutationOptions(repoStockRealisedScrip.Put()).invalidateQueryKey,
    }),
    onSuccess: async ({ invalidateQueryKey }) => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: invalidateQueryKey }),
        queryClient.invalidateQueries({
          queryKey: ApiRequestAdapter.queryOptions(repoRealisedDividend.Get()).invalidateQueryKey,
        }),
      ]);
